...
def board_wait():
    ...
    if route_id:
        ...
        if start_route:
            ...
            recent_boarding_location = get_most_recent_boarding_in_route(route_id=route_id,
                                                                         stop_order=_selected_stop.stop_order)
            # print recent_boarding_location.to_dict()
            if _selected_stop and recent_boarding_location:
                # get the ETA
                _estimated_time = calculate_min_wait_time(from_location=_selected_stop,
                                                          to_location=recent_boarding_location)
...
def api_board_wait():
    ...
    # get the most recent boarding location from the travel history
    recent_boarding_route_location = get_most_recent_boarding_in_route(route_id=route_id,
                                                                       stop_order=close_stop.stop_order)
    # print recent_boarding_route_location.to_dict()
    if close_stop and recent_boarding_route_location:
        # get the ETA
        _eta_data = calculate_min_wait_time(from_location=close_stop, to_location=recent_boarding_route_location)
        return jsonify(success=True, item={'eta': _eta_data, 'closeStop': close_stop.to_dict(),
                                           'recentBoardStop': recent_boarding_route_location.to_dict()})
...
def calculate_min_wait_time(from_location, to_location):
    ...
    if api_key and api_url:
        ...
        url = api_url.format(api_key, from_location.location_lati, from_location.location_longi,
                             to_location.location_lati,
        ...
        return {'from_location': from_location_name, 'to_location': to_location_name, 'duration': duration,
                'distance': distance}
